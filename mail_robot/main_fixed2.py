def main():
    ...
    # Создаём group ПЕРВЫМ (строка 628-631)
    group = parser.add_argument_group("Read & Mark Commands")

    # Потом добавляем `-h/--help` к группе (строка 633)
    group.add_argument(
    ...
    action_flags = [
        ...
        if arg in ["-h", "--help", "-get", "-check", "-all", "-read", "-mark-read"]
    ]

    if not action_flags:
        print(
            "Приложение запущено без команд. Используйте `-h`, `--help` или другой флаг для просмотра списка доступных команд."
        )
        return

    # Process flags - only if -h/--help was explicitly passed
